...
public class PropCardinality extends Propagator<Variable> {
    ...
    public void propagate(int evtmask) throws ContradictionException {
        System.out.println("In prop cardinality for " + set.getName() + "-" + set.isInstantiated()  + " :" + card + "-" + card.isInstantiated());
        int k = set.getLB().size();
        card.updateLowerBound(k, this);
        int e = set.getUB().size();
        card.updateUpperBound(e, this);
        if (card.isInstantiated()) {
            System.out.println("Card is instantiated");
            int c = card.getValue();
            if (c == k) {
                if(set.getName().equals("76[1]")) System.out.println("UB: " + Arrays.toString(set.getUB().toArray()));
                ISetIterator iter = set.getUB().iterator();
                while (iter.hasNext()){
                    int j = iter.nextInt();
                    if(set.getName().equals("76[1]")) System.out.println("Removing " + j + " : " + iter.hasNext());
        ...
        System.out.println("Exit prop cardinality for " + set.getName() + "-" + set.isInstantiated() + ":" + card + "-" + card.isInstantiated());
    }
